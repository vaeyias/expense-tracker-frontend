...
<template>
  <div class="page">
    <div class="content">
      <div class="panel card" style="display:flex; justify-content:space-between; align-items:center; gap:12px; margin-bottom:16px;">
        <div>
          <div class="h1">{{ group?.name }}</div>
          <div style="color:var(--muted); font-size:0.95rem;">{{ group?.description || 'Group details' }}</div>
        </div>
        <div class="row">
...
<div class="expense-row" v-for="expense in expenses" :key="expense._id" @click="openExpenseModal(expense)">
...
<div v-if="showExpenseModal" class="modal-overlay" @click.self="closeExpenseModal">
  <div class="expense-modal panel modal-card" role="dialog" aria-modal="true" aria-label="Expense details">
...
<style scoped>
.expenses-list { display:flex; flex-direction:column; gap:10px; margin-top:10px; background-color: ; }
...
/* container */
.expenses-list { display:flex; flex-direction:column; gap:10px; margin-top:10px; }
...
.expense-row {
  ...
  padding: 12px;
  border-radius: 12px;
  background: linear-gradient(190deg, var(--brand-deep), var(--brand-mid)); /* subtle translucent row background */
...
/* typography / colors */
.expense-title { font-weight:600; color:var(--brand-light); }
.expense-category { color:var(--muted); font-size:0.95rem; }
.expense-description { color:var(--muted); font-size:0.95rem; }
.expense-payer {
  font-weight: 600;
  color: var(--brand-mid);
}
.expense-total { font-weight:700; color:var(--brand-light); }

/* you-owe: muted by default, highlighted when positive */
.expense-youOwe { color: var(--muted); font-weight:600; }
.expense-youOwe.highlight { color: var(--brand-highlight); font-weight:800; }

/* keep the action cell aligned to the right */
.expense-action { display:flex; align-items:center; justify-content:flex-end; }

/* responsive tweaks */
@media (max-width:1000px) {
  .expense-row { grid-template-columns: 110px 1fr 120px 100px; grid-auto-rows: auto; grid-template-areas:
    "date title title action"
    "category description payer action"; }
  .expense-date { grid-area: date; }
  .expense-title { grid-area: title; }
  .expense-category { grid-area: category; }
  .expense-description { grid-area: description; }
  .expense-payer { grid-area: payer; }
  .expense-action { grid-area: action; display:flex; justify-content:flex-end; }
}
/* pointer + hover affordance */
.expense-row:not(.header) { cursor: pointer; }
...
.modal-close:hover { background: rgba(255,255,255,0.03); transform: translateY(-2px); }
...
.you-owe { font-size:0.95rem; color: var(--muted); margin-top:6px; }
.you-owe.highlight { color: var(--brand-highlight); font-weight:800; }
...
.value { font-size: 1rem; color: var(--brand-light); }

/* description larger readable block */
.description { white-space: pre-wrap; line-height:1.45; color: rgba(255,255,255,0.9); }
...
@media (max-width: 880px) {
  ...
  .amount-box { width: 100%; align-self:flex-start; }
  .modal-header { flex-direction: column; align-items:flex-start; gap:10px; }
  .amount-box { min-width: unset; align-items:flex-start; padding-left:12px; }
}

/* small screens reduce padding */
@media (max-width: 420px) {
  .modal-card.expense-modal { padding: 12px; border-radius: 10px; }
...
@media (max-width:600px) {
  .panel { padding:14px; }
  .expense-row { padding:10px; border-radius:10px; }
  .expense-row:not(.header):hover { transform: translateY(-4px); box-shadow: 0 8px 18px rgba(4,10,24,0.14); }
}

.expense-title { font-weight:600; color:var(--brand-highlight);font-size:0.95rem; font-weight:700;}
.expense-category { color:var(--muted); font-size:0.95rem; }
.expense-description { color:var(--muted); font-size:0.95rem; }
.expense-payer { font-weight:600; color:white;font-weight: 700; /* adds an outline */
}
.expense-total { font-weight:700; color:var(--brand); }

.expense-youOwe.danger { color: var(--brand-highlight); font-weight:700; }

.back-btn { display:none; } /* using header controls now */
